...
def test__load_ciritics(critic_fixture):
...
def test__get_critics_for_one_user(critic_fixture):
    assert isinstance(critic_fixture._get_critics_for_one_user(
        'Lisa Rose'), np.ndarray)


def test__get_critics_for_one_item(critic_fixture):
    assert isinstance(critic_fixture._get_critics_for_one_item(
        'Lady in the Water'), np.ndarray)


def test_get_critics_for_one_object(critic_fixture):
    expected = critic_fixture._get_critics_for_one_item('Lady in the Water')
    results = critic_fixture.get_critics_for_one_object(
        'item', 'Lady in the Water')
    assert (expected == results).all()
    expected = critic_fixture._get_critics_for_one_user('Lisa Rose')
